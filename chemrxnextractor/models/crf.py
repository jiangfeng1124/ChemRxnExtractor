...
class ConditionalRandomField(torch.nn.Module):
    ...
    def viterbi_tags(
        ...
        start_tag = num_tags
        end_tag = num_tags + 1
        transitions = torch.Tensor(num_tags + 2, num_tags + 2).fill_(-INFINITY)

        # Apply transition constraints
        constrained_transitions = self.transitions * self._constraint_mask[
            :num_tags, :num_tags
        ] + -INFINITY * (1 - self._constraint_mask[:num_tags, :num_tags])
        ...
        if self.include_start_end_transitions:
            ...
            transitions[:num_tags, end_tag] = self.end_transitions.detach() * self._constraint_mask[
                :num_tags, end_tag
            ].data + -INFINITY * (1 - self._constraint_mask[:num_tags, end_tag].detach())
        else:
            transitions[start_tag, :num_tags] = -INFINITY * (
                1 - self._constraint_mask[start_tag, :num_tags].detach()
            )
            transitions[:num_tags, end_tag] = -INFINITY * (
